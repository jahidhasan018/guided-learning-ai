...
export function Sidebar({ 
  ...
  const theorySessions = sessions.filter(s => s.mode === 'Theory');
  const projectSessions = sessions.filter(s => s.mode === 'Project');

  const renderSessionItem = (session: LearningSession) => (
    <div
      key={session.id}
      className={cn(
              ...
              {theorySessions.length > 0 && (
                <div className="space-y-1">
                  <div className="px-3 mb-2 flex items-center gap-2">
                    ...
                    <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Theory</h2>
                  </div>
                  {theorySessions.map(renderSessionItem)}
                </div>
              )}
            </>
          )}
